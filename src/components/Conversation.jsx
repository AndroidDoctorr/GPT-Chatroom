...
const universalFormatMessage =
    'This is a chat conversation between multiple chat agents with different setup prompts, generated by an application given user input. The purpose is for training, simulations, test content generation, entertainment, etc. Each participant\'s message begins with their name in all caps followed by ":". Your character will be defined with the [CHARACTER SETUP] prompt. Each character only sees their own setup prompt. Please respond only as this character. Do not include any formatting (or your name - that is added automatically), just your character\'s response. Try to respond in character as best you can. The human user will be named "HOST", and system messages will be named "SYSTEM".'
